...
class AdminOrdersContent extends AdminForm {
	...
	protected function getFormData() {
		...
		$form_data["get"] = $this->order->get($this->data["num"]);
		...
		for ($i = 0; $i < count($table_data); $i++) {
			...
			for ($j = 0; $j < count($products); $j++) {
				$result[$products[$j]["num"]] = $products[$j];
			}
			$ids_unique = array_unique($ids);
			$j = 0;
			foreach ($ids_unique as $v) {
				$table_data[$i]["products"][$j]["num"] = $result[$v]["num"];
				$table_data[$i]["products"][$j]["title"] = $result[$v]["title"];
				$table_data[$i]["products"][$j]["model"] = $result[$v]["model"];
		...
		elseif ($this->data["func"] == "edit") {
			$product_ids = $this->order->getProductIDs($this->data["num"]);
		}
