		...
		<tr>
			...
			<td>
				<?php foreach ($data["products"] as $product) { ?>
					<p><?=$product["title"]?> (<?=$product["model"]?>)<br />Код товара: <?=$product["num"]?> - <?=$product["count"]?> шт.</p>
				<?php } ?>
			</td>
			<td><?=$data["price"]?></td>
			<td><?=$data["name"]?></td>
			<td><?=$data["phone"]?></td>
			<td><?=$data["email"]?></td>
			<td><?php if ($data["delivery"] == 0) { ?>В отделение "Новой почты"<?php } else { ?>Курьерская доставка<?php } ?></td>
			<td><?=$data["address"]?></td>
			<td><?=$data["notice"]?></td>
