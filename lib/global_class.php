...
abstract class GlobalClass {
	...
	public function edit($num, $data) {
		if (!$this->check($data)) return false;
		$query = "UPDATE `".$this->table_name."` SET ";
		foreach ($data as $field => $value) $query .= "`$field` = ".$this->config->sym_query.",";
		$query = substr($query, 0, -1);
		$query .= "WHERE `num` = ".$this->config->sym_query;
		$data["num"] = $num;
		return $this->db->query($query, array_values($data));
	}
	
	public function delete($num) {
		$query = "DELETE FROM `".$this->table_name."` WHERE `num`=".$this->config->sym_query;
		return $this->db->query($query, array($num));
	}
	...
	public function existsID($num) {
		if (!$this->check->num($num)) return false;
		return $this->isExistsFV("num", $num);
	}
	...
	public function get($num) {
		if (!$this->check->num($num)) return false;
		return $this->getOnField("num", $num);
	}
	...
	protected function getFieldOnID($num, $field) {
		return $this->getField("num", $num, $field);
	}
	...
	protected function setFieldOnID($num, $field, $value) {
		return $this->setField("num", $num, $field, $value);
	}
	...
	public function getCount() {
		$query = "SELECT COUNT(`num`) FROM `".$this->table_name."`";
